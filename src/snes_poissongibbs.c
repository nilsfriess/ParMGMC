 ...
 */
static PetscScalar find_argmax_phi(const PetscScalar mu_bar,
                                   const PetscScalar sigma,
                                   const PetscInt n_k,
                                   const PetscScalar* nu,
                                   const PetscScalar* b) {
...
static PetscErrorCode SNESSetUp_PoissonGibbs(SNES snes)
{
  ...
  // random numbers
  if (!poissongibbs->prand)
    PetscCall(ParMGMCGetPetscRandom(&poissongibbs->prand));
  if (!poissongibbs->random_workspace)
    PetscCall(VecCreateSeq(PETSC_COMM_SELF, RANDOM_BUFFER_SIZE,
                           &poissongibbs->random_workspace));
