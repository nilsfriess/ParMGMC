...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The MALA sampler \cite{roberts1996exponential} implements the update $\vec{\theta}\mapsto\vec{\theta}'$ by first drawing a proposal $\vec{\theta}^*$ from a multivariant normal distribution by setting
\begin{equation}
\vec{\theta}^*=\vec{\theta}+\frac{\epsilon^2}{2} M^{-1}\nabla_\theta \log(p(\vec{\theta}))+\epsilon \vec{\xi}\qquad\text{with $\vec{\xi}\sim \mathcal{N}(\vec{0},M^{-1})$}\label{eqn:langevin_proposal}
\end{equation}
and then accepting it with the probability
\begin{equation}
\alpha = \min\left\{\frac{p(\vec{\theta}^*)\pi(\vec{\theta}^*\gets\vec{\theta})}{p(\vec{\theta})\pi(\vec{\theta}\gets\vec{\theta}^*)}\right\}
\end{equation}
where $p = p(\cdot|\vec{n};Q,B,\vec{f},\vec{\nu})$ is the posterior density defined in~\eqref{eqn:posterior_density}. $\pi(\vec{\theta}^*\gets\vec{\theta})$ is the density for the transion \eqref{eqn:langevin_proposal}, namely
\begin{equation}
    \pi(\vec{\theta}^*\gets\vec{\theta}) = \mathcal{Z}\exp\left[-\frac{1}{2\epsilon^2}(\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))^\top M (\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))\right]\qquad\text{with $\vec{\Phi}(\vec{\theta})=\vec{\theta}+\frac{\epsilon^2}{2}M^{-1}\nabla_\theta \log(p(\vec{\theta}))$}.
\end{equation}
In the simplest setup, matrix $M$ is the $n$-dimensional identity matrix. A better choice would be the precision matrix $Q$ of the prior or the Fisher information matrix, namely
\begin{equation}
M_{ij} = - \frac{\partial^2 }{\partial \theta_i \partial \theta_j} \log(p(\vec{\theta})).\label{eqn:fisher_information_matrix}
\end{equation}
However, in this case $M$ in no longer independent of $\vec{\theta}$, which leads to seveal complications: the Langevin proposal in~\eqref{eqn:langevin_proposal} needs to modified and the normalisation constant no longer cancels in the contribution $\pi(\vec{\theta}^*\gets\vec{\theta})/\pi(\vec{\theta}\gets\vec{\theta}^*)$. To address this, we will use a fixed value of $\vec{\theta}$ in~\eqref{eqn:fisher_information_matrix} as described below.
...
&= Q + B Z B^\top.
    \end{aligned}
\end{equation}
where $Z$ is the diagonal $m\times m$ matrix with $Z_{kk} = Z_{kk}(\vec{\theta}) = t_k r_k(\vec{\theta})$. Since $r_k(\vec{\theta})$ is the rate, $t_kr_k(\vec{\theta})$ is the number of events counted at site $k$, which can be estimated as $n_k$. To obtain a constant preconditioning matrix we therefore set
\begin{equation}
...
&= \left(\mathbb{I} - GB^\top\right) Q^{-1}
    \end{aligned}
\end{equation}
where $G = \overline{B} \left(Z^{-1} + B^\top \overline{B}\right)^{-1}$ and the $n\times m$ matrix $\overline{B} = Q^{-1} B$ is obtained by solving $m$ linear systems with the columns $\vec{b}^{(k)}$ of $B$ on the right hand side.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
