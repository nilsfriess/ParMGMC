...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Suppose that instead of sampling $\vec{\theta}$ directly, we are given some initial $\vec{\theta}_0$ and want to sample $\delta\vec{\theta}\sim p_\delta$ such that $\vec{\theta}=\vec{\theta}_0+\delta\vec{\theta}$ is distributed according to $p(\cdot|\vec{n};Q,B,\vec{f},\vec{\nu})$ in~\eqref{eqn:posterior_density}. For a given $\vec{\theta}_0$ the distribution $p_\delta$ can be obtained as follows. First observe that
\begin{equation}
    \begin{aligned}
...
\begin{algorithm}
    ...
    \textit{Input}: Previous state $\vec{\theta}_0$, measurements $\vec{n}\in\mathbb{N}^m$, SPD prior matrix $Q\in\mathbb{R}^{n\times n}$, measurement matrix $B\in\mathbb{R}^{n\times m}$, right hand side vector $\vec{f}\in\mathbb{R}^n$, bias vector $\vec{\nu}\in\mathbb{R}^m$\\
    \textit{Output:} Updated state $\vec{\theta}\in\mathbb{R}^n$}\label{alg:incrementalsampler}
\begin{algorithmic}[1]
\Function{IncrementalPosteriorSampler}{$\vec{\theta}_0;\vec{n},Q,B,\vec{f},\vec{\nu}$}
...
where
\begin{equation}
\widetilde{\nu}_k^{(i)} = \nu_k - \sum_{j\neq i} B_{jk} \theta_j = \underbrace{\Big(\nu_k - \sum_{j} B_{jk} \theta_j\Big)}_{=:\widetilde{\nu}_k} + B_{ik} \theta_i
\end{equation}
and
\begin{equation}
...
&= \sum_{k\in\mathcal{M}_i}e^{B_{ik}\theta-\widetilde{\nu}_k^{(i)}}
+ \sum_{k\in\mathcal{M}_i} \left((\overline{\theta}_i-\theta)B_{ik} -1\right) e^{B_{ik}\overline{\theta}_i-\widetilde{\nu}_k^{(i)}}
    \end{aligned}
\end{equation}
...
\begin{algorithm}
    ...
    \textit{Input}: Current state $\vec{\theta}\in\mathbb{R}^n$; measurements $\vec{n}\in\mathbb{N}^m$, SPD prior matrix $Q\in\mathbb{R}^{n\times n}$, measurement matrix $B\in\mathbb{R}^{n\times m}$, right hand side vector $\vec{f}\in\mathbb{R}^n$, bias vector $\vec{\nu}\in\mathbb{R}^m$ \\
    \textit{Output:} Updated state $\vec{\theta}\in\mathbb{R}^n$ (update in place)}
\begin{algorithmic}[1]
\Procedure{PoissonGibbs}{$\vec{\theta};\vec{n},Q,B,\vec{f},\vec{\nu}$}
\For{$i=1,2,\dots,n$}
...
\begin{algorithm}[H]
    \caption{Multigrid Monte Carlo (non-incremental version)\\
    \textit{Input}: Current state $\vec{\theta}_i$, observed rates $\vec{n}$, system matrix $A$ (consisting of precision matrix and observation matrix $B$), vector $\vec{b}$ (consisting of right hand side $\vec{f}$ and offsets $\vec{\nu}$), Samplers $\mathcal{S}_{\text{pre}}$, $\mathcal{S}_{\text{post}}$ and $\mathcal{S}_{\text{coarse}}$, intergrid operators $P$, $R$ and $\hat{R}$\\
    \textit{Output:} Updated state $\vec{\theta}_{i+1}$}\label{alg:mgmc}
\begin{algorithmic}[1]
    \Function{MGMC}{$\vec{\theta}_i;\vec{n},A,\vec{b}$}
...
\begin{algorithm}[H]
    \caption{Full Approximation Scheme (FAS)\\
...
\end{itemize} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{MALA sampling}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The MALA sampler \cite{roberts1996exponential} implements the update $\vec{\theta}\mapsto\vec{\theta}'$ by first drawing a proposal $\vec{\theta}^*$ from a multivariate normal distribution by setting
\begin{equation}
\vec{\theta}^*=\vec{\Phi}(\vec{\theta})+\epsilon \vec{\xi} \qquad\text{with $\vec{\Phi}(\vec{\theta})=\vec{\theta}+\frac{\epsilon^2}{2}M^{-1}\nabla_\theta \log(p(\vec{\theta}))$ and $\vec{\xi}\sim \mathcal{N}(\vec{0},M^{-1})$}.
\label{eqn:langevin_proposal}
\end{equation}
This proposal is accepted with the Metropolis Hasting \cite{metropolis1953equation,hastings1970monte} probability
\begin{equation}
\alpha = \min\left\{\frac{p(\vec{\theta}^*)\pi(\vec{\theta}^*|\vec{\theta})}{p(\vec{\theta})\pi(\vec{\theta}|\vec{\theta}^*)},1\right\}
\end{equation}
where $p = p(\cdot|\vec{n};Q,B,\vec{f},\vec{\nu})$ is the posterior density defined in~\eqref{eqn:posterior_density} and $\pi(\vec{\theta}^*|\vec{\theta})$ is the density for the transition \eqref{eqn:langevin_proposal}, namely
\begin{equation}
    \pi(\vec{\theta}^*|\vec{\theta}) = \mathcal{Z}\exp\left[-\frac{1}{2\epsilon^2}(\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))^\top M (\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))\right].
\end{equation}
In the simplest setup, the matrix $M$ is the $n$-dimensional identity matrix. A better choice would be the precision matrix $Q$ of the prior or the Fisher information matrix, namely
\begin{equation}
M_{ij} = - \frac{\partial^2 }{\partial \theta_i \partial \theta_j} \log(p(\vec{\theta})).\label{eqn:fisher_information_matrix}
\end{equation}
However, in this case $M$ in no longer independent of $\vec{\theta}$, which leads to several complications: the Langevin proposal in~\eqref{eqn:langevin_proposal} needs to modified and the normalisation constant no longer cancels in the ratio $\pi(\vec{\theta}^*|\vec{\theta})/\pi(\vec{\theta}|\vec{\theta}^*)$ which enters the acceptance rate. To address this, we will use a fixed value of $\vec{\theta}$ in~\eqref{eqn:fisher_information_matrix} as described below.

From~\eqref{eqn:posterior_density} we obtain
\begin{equation}
\log(p(\vec{\theta})) = -\frac{1}{2} (\vec{\theta}-\vec{\mu})^\top Q (\vec{\theta}-\vec{\mu}) + \sum_{k=1}^{m} \left(n_k\log(r_k(\vec{\theta})) - t_k r_k(\vec{\theta})\right) + \text{constant}
\end{equation}
where $\log(r_k(\theta_k)) = (\vec{b}^{(k)})^\top \vec{\theta} +\beta$. This gives
\begin{equation}
    \begin{aligned}
\nabla_\theta \log(p(\vec{\theta})) &= Q(\vec{\mu}-\vec{\theta}) + \sum_{k=1}^{m} \left(n_k - t_k r_k(\vec{\theta})\right)\vec{b}^{(k)}\\
&= \vec{f}-Q\vec{\theta} + B \vec{\sigma}
\qquad\text{with $\sigma_k=\sigma_k(\vec{\theta}) = n_k - t_k r_k(\vec{\theta})$}.
    \end{aligned}
\end{equation}
The (negative) Hessian is 
\begin{equation}
    \begin{aligned}
-\nabla^2_\theta \log(p(\vec{\theta})) &= Q + \sum_{k=1}^{m} t_k r_k(\vec{\theta})\vec{b}^{(k)} (\vec{b}^{(k)})^\top\\
&= Q + B Z B^\top.
    \end{aligned}
\end{equation}
where $Z$ is the diagonal $m\times m$ matrix with $Z_{kk} = Z_{kk}(\vec{\theta}) = t_k r_k(\vec{\theta})$. Since $r_k(\vec{\theta})$ is the rate at site $k$, $t_kr_k(\vec{\theta})$ is the number of events counted at this site, which can be estimated as $n_k$. To obtain a constant preconditioning matrix we therefore set
\begin{equation}
M = Q + B Z B^\top\qquad\text{with $Z_{kk} = n_k$, $Z_{k\ell}=0$ for $k\neq \ell$.}
\end{equation}
The computation of $\vec{\Phi}(\vec{\theta})$ requires the solution of a linear system $M \vec{u} = \vec{r}$. Since $M$ is the sum of a sparse $n\times n$ matrix and a rank-$m$ correction, the Woodbury matrix identity can be used to write
\begin{equation}
    \begin{aligned}
M^{-1} &= Q^{-1} - Q^{-1} B\left(Z^{-1} + B^\top Q^{-1} B\right)^{-1} B^\top Q^{-1}\\
&= \left(\mathbb{I} - GB^\top\right) Q^{-1}
    \end{aligned}
\end{equation}
where
\begin{equation}
G = \overline{B} \left(Z^{-1} + B^\top \overline{B}\right)^{-1}\label{eqn:def_G}
\end{equation}
and the $n\times m$ matrix $\overline{B} = Q^{-1} B$ is obtained by solving $m$ linear systems with the columns $\vec{b}^{(k)}$ of $B$ on the right hand side. With this, a sample $\vec{\xi}$ from $\mathcal{N}(0,M^{-1})$ can be drawn according to~\cref{alg:low_rank_sampler}, as is shown in~\cref{sec:low_rank_sampler}.
\begin{algorithm}[H]
    \caption{Sampler for $\mathcal{N}(0,M^{-1})$ with $M = Q + B Z B^\top$\\
    \textit{Input}: Prior precision matrix $Q\in \mathbb{R}^{n\times n}$, measurement matrix $B\in \mathbb{R}^{n\times m}$, diagonal matrix $Z\in\mathbb{R}^{m\times m}$, matrix $G\in\mathbb{R}^{n\times m}$ defined in~\eqref{eqn:def_G}.\\
    \textit{Output:} Sample $\vec{\xi}\sim \mathcal{N}(0,M^{-1})$\label{alg:low_rank_sampler}}
\begin{algorithmic}[1]
    \Function{MALANoiseSampler}{$Q,B,Z,G$}
    \State{Draw $\vec{\zeta}\sim \mathcal{N}(0,\mathbb{I}_{m})$}
    \State{Compute right hand side $\vec{f}^* = B Z^{1/2} \vec{\zeta}$}
    \State{Solve $Q\vec{\mu}^*=\vec{f}^*$ for $\vec{\mu}^*$}
    \State{Draw $\vec{\xi}^* \sim \mathcal{N}(\vec{\mu}^*,Q^{-1})$}
    \State{Set $\vec{\xi} = \vec{\xi}^* - G(B^\top \vec{\xi}^*)$}
    \State{\Return $\vec{\xi}$}
    \EndFunction
\end{algorithmic}
\end{algorithm}
To compute the acceptance ratio, note that $\log(\alpha) = \delta_1+\delta_2$ with
\begin{equation}
    \begin{aligned}
        \delta_1 := \log(p(\vec{\theta}^*)/p(\vec{\theta})) &= \frac{1}{2} \left(\vec{\theta}^\top Q \vec{\theta}- (\vec{\theta}^*)^\top Q \vec{\theta}^*\right) + \vec{f}^\top (\vec{\theta}^*-\vec{\theta})\\
        &\quad+\;\; \sum_{k=1}^m \left(n_k \log(\Lambda_k(\vec{\theta}^*)/\Lambda_k(\vec{\theta})) + \Lambda_k(\vec{\theta}) - \Lambda_k(\vec{\theta}^*)  \right)\\
        &= \frac{1}{2} \left(\vec{\theta}^\top Q \vec{\theta}- (\vec{\theta}^*)^\top Q \vec{\theta}^*\right) + \vec{f}^\top (\vec{\theta}^*-\vec{\theta})\\
        &\quad+\;\; \sum_{k=1}^m \left(n_k (\vec{b}^{(k)})^\top(\vec{\theta}^*-\vec{\theta}) + \exp\left[(\vec{b}^{(k)})^\top \vec{\theta}-\nu_k\right] - \exp\left[(\vec{b}^{(k)})^\top \vec{\theta}^*-\nu_k\right] \right)\\
        &= \frac{1}{2} \left(\vec{\theta}^\top Q \vec{\theta}- (\vec{\theta}^*)^\top Q \vec{\theta}^*\right) + \vec{f}^\top (\vec{\theta}^*-\vec{\theta})\\
        &\quad+\;\; \vec{n}^\top \vec{B}^\top (\vec{\theta}^*-\vec{\theta}) + \sum_{k=1}^m e^{-\nu_k}\left(\exp\left[(\vec{b}^{(k)})^\top \vec{\theta}\right] - \exp\left[(\vec{b}^{(k)})^\top \vec{\theta}^*\right] \right)
    \end{aligned}\label{eqn:delta_1}
\end{equation}
and 
\begin{equation}
    \begin{aligned}
        \delta_2 := \log(\pi(\vec{\theta}^*|\vec{\theta})/\pi(\vec{\theta}|\vec{\theta}^*)) &=
        \frac{1}{2\epsilon^2} \left((\vec{\theta}-\vec{\Phi}(\vec{\theta}^*))^\top M (\vec{\theta}-\vec{\Phi}(\vec{\theta}^*)) - (\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))^\top M (\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))\right)\\
        &= \frac{1}{2\epsilon^2} \left( (\vec{\Delta}^*)^\top Q \vec{\Delta}^* - \vec{\Delta}^\top Q \vec{\Delta} + (B^\top \vec{\Delta}^*)^\top Z (B^\top \vec{\Delta}^*) - (B^\top \vec{\Delta})^\top Z (B^\top\vec{\Delta})\right)
    \end{aligned}\label{eqn:delta_2}
\end{equation}
where
\begin{xalignat}{2}
    \vec{\Delta} &:= \vec{\theta}^* - \vec{\Phi}(\vec{\theta}),&
    \vec{\Delta}^* &:= \vec{\theta} - \vec{\Phi}(\vec{\theta}^*).
\end{xalignat}
\begin{algorithm}[H]
    \caption{Compute bias $\vec{\Phi}(\vec{\theta})$ in MALA proposal~\eqref{eqn:langevin_proposal}\\
    \textit{Input}: State $\vec{\theta}$, measurement vector $\vec{n}\in\mathbb{R}^m$, prior precision matrix $Q\in \mathbb{R}^{n\times n}$, measurement matrix $B\in \mathbb{R}^{n\times m}$, matrix $G\in\mathbb{R}^{n\times m}$ defined in~\eqref{eqn:def_G}, right hand side $\vec{f}\in\mathbb{R}^n$, offset vector $\vec{\nu}\in\mathbb{R}^m$, MALA step size $\epsilon$.\\
    \textit{Output:} $\vec{\Phi}(\vec{\theta})$\label{alg:MALA_bias}}
\begin{algorithmic}[1]
    \Function{$\vec{\Phi}$}{$\vec{\theta};\vec{n},Q,B,G,\vec{f},\vec{\nu},\epsilon$}
    \State {Compute $\vec{\sigma}$ with $\sigma_k = n_k - t_k r_k(\vec{\theta}) = n_k - \exp[(\vec{b}^{(k)})^\top \vec{\theta} - \nu_k]$ for $k=1,2,\dots,m$}
    \State {Set $\vec{s} = \vec{f}-Q\vec{\theta} + B\vec{\sigma}$}
    \State {Solve $Q \vec{\tau} = \vec{s}$ for $\vec{\tau}$}
    \State {Set $\vec{\Phi} = \vec{\theta} + \frac{\epsilon^2}{2}\left(\vec{\tau} - G(B^\top \vec{\tau})\right)$}
    \State{\Return $\vec{\Phi}$}    
    \EndFunction
\end{algorithmic}
\end{algorithm}
The full preconditioned MALA sampler is written down in~\cref{alg:preconditioned_mala_sampler}. Obviously, the construction of the matrices $Z$ and $G$ is only required once at setup.
\begin{algorithm}[H]
    \caption{Preconditioned MALA sampler\\
    \textit{Input}: Current state $\vec{\theta}$, measurement vector $\vec{n}\in\mathbb{R}^n$, prior precision matrix $Q\in \mathbb{R}^{n\times n}$, measurement matrix $B\in \mathbb{R}^{n\times m}$, right hand side $\vec{f}\in\mathbb{R}^n$, offset vector $\vec{\nu}\in\mathbb{R}^m$, step size $\epsilon$\\
    \textit{Output:} New sample $\vec{\theta}'$\label{alg:preconditioned_mala_sampler}}
\begin{algorithmic}[1]
    \Function{PreconditionedMALASampler}{$\vec{n},Q,B,\vec{f},\vec{\nu},\epsilon$}
    \State {Construct the diagonal $m\times m$ matrix $Z$ with $Z_{k\ell} = n_k \delta_{k\ell}$.}
    \For {$k=1,2,\dots,m$}
        \State {Solve $Q\overline{\vec{b}}^{(k)} = \vec{b}^{(k)}$ for $\overline{\vec{b}}^{(k)}$}
    \EndFor
    \State {Set $\overline{B} = \left(\overline{\vec{b}}^{(1)}|\overline{\vec{b}}^{(2)}|\dots|\overline{\vec{b}}^{(m)}\right).$}
    \State {Construct the $n\times m$ matrix $G$ according to~\eqref{eqn:def_G}.}
    \State {Compute bias $\vec{\phi} = \vec{\Phi}(\vec{\theta};\vec{n},Q,B,G,\vec{f},\vec{\nu},\epsilon)$ in~\eqref{eqn:langevin_proposal} with~\cref{alg:MALA_bias}.}
    \State{Draw $\vec{\xi}\sim \mathcal{N}(0,M^{-1})$ with~\cref{alg:low_rank_sampler}.}
    \State{Set proposal $\vec{\theta}^*=\vec{\phi} + \epsilon \vec{\xi}$.}
    \State{Compute $\vec{\phi}^* = \vec{\Phi}(\vec{\theta}^*;\vec{n},Q,B,G,\vec{f},\vec{\nu},\epsilon)$ with~\cref{alg:MALA_bias}.}
    \State{Compute $\delta = \delta_1 + \delta_2$ according to~\eqref{eqn:delta_1} and~\eqref{eqn:delta_2}.}
    \If {$\delta \ge 0$}
        \State{Set $\vec{\theta}' = \vec{\theta}^*$.}
    \Else
...
\appendix
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Low rank sampler}\label{sec:low_rank_sampler}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
To show that~\cref{alg:low_rank_sampler} indeed produces samples from the desired distribution, first observe that
\begin{equation}
\vec{\xi}^* = Q^{-1} B Z^{1/2} \vec{\zeta} + Q^{-1/2} \vec{\omega}\qquad\text{with $\vec{\zeta}\sim\mathcal{N}(0,\mathbb{I}_m)$ and $\vec{\omega}\sim\mathcal{N}(0,\mathbb{I}_n)$.}
\end{equation}
Further, since $\mathbb{I}-GB^\top = M^{-1}Q$ we also have
\begin{equation}
\vec{\xi} = M^{-1} \left(B Z^{1/2} \vec{\zeta} + Q^{1/2} \vec{\omega}\right).
\end{equation}
Obviously $\vec{\xi}$ is Gaussian with mean zero. Further, since $\vec{\zeta}$ and $\vec{\omega}$ are independent, the covariance of $\vec{\xi}$ is
\begin{equation}
    \begin{aligned}
\mathbb{E}[\vec{\xi}\vec{\xi}^\top] &= M^{-1} \mathbb{E}\left[\left(B Z^{1/2} \vec{\zeta} + Q^{1/2} \vec{\omega}\right)\left(\vec{\zeta}^\top Z^{1/2} B^\top +  \vec{\omega}^\top Q^{1/2}\right)\right]M^{-1}\\
&= M^{-1} \left(B Z^{1/2} \mathbb{E}\left[\vec{\zeta}\vec{\zeta}^\top\right]Z^{1/2} B^\top + Q^{1/2} \mathbb{E}\left[\vec{\omega}\vec{\omega}^\top\right] Q^{1/2}\right)M^{-1}\\
&= M^{-1}\left(BZB^\top + Q\right)M^{-1} \\
&= M^{-1}.
    \end{aligned}
\end{equation}
Hence, as required $\vec{\xi}\sim\mathcal{N}(0,M^{-1})$.
\bibliographystyle{unsrt}
\bibliography{poissongibbs}
\end{document}