...
\begin{algorithmic}[1]
\Function{IncrementalPosteriorSampler}{$\vec{\theta}_0;\vec{n},Q,B,\vec{\nu}$}
...
\begin{algorithmic}[1]
\Procedure{PoissonGibbs}{$\vec{\theta};\vec{n,Q,B,\vec{\nu}}$}
\For{$i=1,2,\dots,n$}
...
\end{equation}
and the $n\times m$ matrix $\overline{B} = Q^{-1} B$ is obtained by solving $m$ linear systems with the columns $\vec{b}^{(k)}$ of $B$ on the right hand side. With this, a sample $\vec{\xi}$ from $\mathcal{N}(0,M^{-1})$ can be drawn according to~\cref{alg:low_rank_sampler}.
\begin{algorithm}[H]
...
\begin{algorithmic}[1]
    ...
    \State{Draw $\vec{\xi}^* \sim \mathcal{N}(\vec{f}^*,Q^{-1})$}
...
\begin{equation}
    \begin{aligned}
        \delta_1 := \log(p(\vec{\theta}^*)/p(\vec{\theta})) &= \frac{1}{2} \left((\vec{\theta}-\vec{\mu})^\top Q (\vec{\theta}-\vec{\mu})- (\vec{\theta}^*-\vec{\mu})^\top Q (\vec{\theta}^*-\vec{\mu})\right)\\
        &\quad+\;\; \sum_{k=1}^m \left(n_k \log(\Lambda_k(\vec{\theta}^*)/\Lambda_k(\vec{\theta})) + \Lambda_k(\vec{\theta}) - \Lambda_k(\vec{\theta}^*)  \right)\\
        &= \frac{1}{2} \left((\vec{\theta}-\vec{\mu})^\top Q (\vec{\theta}-\vec{\mu})- (\vec{\theta}^*-\vec{\mu})^\top Q (\vec{\theta}^*-\vec{\mu})\right)\\
...
\begin{algorithm}[H]
    ...
    \textit{Input}: State $\vec{\theta}$, measurement vector $\vec{n}\in\mathbb{R}^m$, prior precision matrix $Q\in \mathbb{R}^{n\times n}$, offset $\vec{\mu}\in\mathbb{R}^n$, measurement matrix $B\in \mathbb{R}^{n\times m}$, matrix $G\in\mathbb{R}^{n\times m}$ defined in~\eqref{eqn:def_G},  offset vector $\vec{\nu}\in\mathbb{R}^m$, MALA step size $\epsilon$.\\
    \textit{Output:} $\vec{\Phi}(\vec{\theta})$\label{alg:MALA_bias}}
\begin{algorithmic}[1]
    \Function{$\vec{\Phi}$}{$\vec{\theta};\vec{n},Q,\vec{\mu},B,G,\vec{\nu},\epsilon$}
    \State {Compute $\vec{\sigma}$ with $\sigma_k = n_k - t_k r_k(\vec{\theta}) = n_k - \exp[(\vec{b}^{(k)})^\top \vec{\theta} - \nu_k]$ for $k=1,2,\dots,m$}
    \State {Set $\vec{s} = Q(\vec{\mu}-\vec{\theta}) + B\vec{\sigma}$}
    \State {Solve $Q \vec{\tau} = \vec{s}$}
