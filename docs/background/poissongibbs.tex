...
\end{algorithmic}
\end{algorithm}
To compute the acceptance ratio, note that
\begin{equation}
\log(\alpha) = \delta^{(1)}+\frac{\delta^{(2)}}{2\epsilon^2}\label{eqn:alpha}
\end{equation}
 with
\begin{equation}
    \begin{aligned}
        \delta^{(1)} := \log(p(\vec{\theta}^*)/p(\vec{\theta})) &= \frac{1}{2} \left(\vec{\theta}^\top Q \vec{\theta}- (\vec{\theta}^*)^\top Q \vec{\theta}^*\right) + \vec{f}^\top (\vec{\theta}^*-\vec{\theta})\\
...
\begin{equation}
    \begin{aligned}
        \delta^{(2)} := \log(\pi(\vec{\theta}|\vec{\theta}^*)/\pi(\vec{\theta}^*|\vec{\theta})) &=
         (\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))^\top M (\vec{\theta}^*-\vec{\Phi}(\vec{\theta}))-(\vec{\theta}-\vec{\Phi}(\vec{\theta}^*))^\top M (\vec{\theta}-\vec{\Phi}(\vec{\theta}^*))\\
        &=  \vec{\Delta}^\top Q \vec{\Delta} + (B^\top \vec{\Delta})^\top Z (B^\top \vec{\Delta})- (\vec{\Delta}^*)^\top Q \vec{\Delta}^*  - (B^\top \vec{\Delta}^*)^\top Z (B^\top\vec{\Delta}^*)
...
\begin{algorithmic}[1]
    ...
    \State{Compute $\delta = \delta^{(1)} + \delta^{(2)}/(2\epsilon^2)$ according to~\eqref{eqn:alpha},~\eqref{eqn:delta_1} and~\eqref{eqn:delta_2}.}
    \If {$\delta \ge 0$}
